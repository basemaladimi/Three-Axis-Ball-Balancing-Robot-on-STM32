...
cv2.createTrackbar("Ki", "Captured Frame", 0, 3000, Nothing)
cv2.createTrackbar("Kd", "Captured Frame", 0, 400, Nothing)
cv2.createTrackbar("Ref U", "Captured Frame", 80, 160, Nothing)
cv2.createTrackbar("Ref X", "Captured Frame", 80, 160, Nothing)
cv2.createTrackbar("Ref V", "Captured Frame", 80, 160, Nothing)
...
def draw_axes(frame, center, motors, rB):
    ...
    for i, motor in enumerate(motors):
        ...
        AxisVal = ["U", "X", "V"]
        cv2.putText(frame, AxisVal[i], tuple((motor+10).astype(int)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, colors[i], 2)
...
while True:
    ...
    RefU = cv2.getTrackbarPos("Ref U", "Captured Frame")
    RefX = cv2.getTrackbarPos("Ref X", "Captured Frame")
    RefV = cv2.getTrackbarPos("Ref V", "Captured Frame")
    ...
    if rB != 0 and ball_detected:
        ...
        msg = "{},{},{},{},{},{},{},{},{}\n".format(axes_int[0], axes_int[1], axes_int[2], Kp, Ki, Kd, ValU, ValX, ValV)
    else:
        msg = "0,0,0,{},{},{}\n".format(Kp, Ki, Kd)
